...
- Integration (local Supabase, `@vitest-environment node`): signup provisioning (2) — the real
  002 trigger creates agency + principal user; RLS isolation confirms agency A cannot see B.
...
## Next up
- **A-02** — log in / log out (session persists; logout clears it).
- **A-03** — dedicated RLS isolation test. Largely proven already by A-01's integration test;
  A-03 formalizes it across more tables.
- **A-04** — empty dashboard shell after sign-up (replace the placeholder).
- Human: manual click-through of signup to approve A-01; apply migrations to cloud; F-08.
